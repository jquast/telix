...
def bar_slot(raw, maxval, width, kind, priority=1, order=0):
    flash_elapsed = -1.0
    frags = vital_bar(raw, maxval, width, kind, flash_elapsed=flash_elapsed)
...
class TestFillToolbarGrows:
    ...
    def test_bar_grows_with_extra_space(self):
        s = bar_slot(50, 100, 20, "hp")
        orig_w = s.width
        left, right, sep = fill_toolbar([s], [], orig_w + 40)
    ...
    def test_room_name_grows(self):
        ...
        left, right, sep = fill_toolbar([], [s], 60)
...
class TestBarGapWidth:
    ...
    def test_adjacent_bars_use_bar_gap(self):
        ...
        gaps = left_sep_widths([hp, mp])
        assert gaps == [BAR_GAP_WIDTH]
    ...
    def test_three_bars_all_tight(self):
        hp = bar_slot(50, 100, 20, "hp", order=0)
        mp = bar_slot(30, 80, 20, "mp", order=1)
        xp = bar_slot(10, 50, 20, "xp", order=2)
        gaps = left_sep_widths([hp, mp, xp])
        assert gaps == [BAR_GAP_WIDTH, BAR_GAP_WIDTH]
...
class TestLayoutToolbarReturnsSlots:
    ...
    def test_returns_toolbar_slots(self):
        ...
        assert len(left) == 1
        assert isinstance(left[0], ToolbarSlot)
        assert left[0].fragments == s.fragments

    def test_left_right_ordering(self):
        l1 = text_slot("A", priority=1, order=0, side="left")
        r1 = text_slot("B", priority=2, order=10, side="right")
        left, right = layout_toolbar([l1, r1], 80)
        assert len(left) == 1
        assert len(right) == 1
