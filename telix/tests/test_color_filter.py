"""Tests for telix.color_filter -- ANSI color palette translation."""

# 3rd party
import pytest

# local
from telix.color_filter import (
    PALETTES,
    ColorConfig,
    ColorFilter,
    PetsciiColorFilter,
...
def test_foreground_color_used_on_reset() -> None:
    fg = (200, 180, 160)
    f = ColorFilter(ColorConfig(foreground_color=fg))
    result = f.filter("\x1b[0m")
    assert f"38;2;{fg[0]};{fg[1]};{fg[2]}" in result


def test_foreground_color_used_on_sgr39() -> None:
    fg = (200, 180, 160)
    f = ColorFilter(ColorConfig(foreground_color=fg))
    f.filter("x")
...
def test_foreground_color_used_on_empty_reset() -> None:
    fg = (200, 180, 160)
    f = ColorFilter(ColorConfig(foreground_color=fg))
    result = f.filter("\x1b[m")
    assert f"38;2;{fg[0]};{fg[1]};{fg[2]}" in result
...
def test_foreground_color_unbold_restores_detected_fg() -> None:
    fg = (200, 180, 160)
    f = ColorFilter(ColorConfig(foreground_color=fg))
    f.filter("x")
    f.filter("\x1b[1m")
...
def test_foreground_color_bold_still_promotes_to_bright() -> None:
    fg = (200, 180, 160)
    f = ColorFilter(ColorConfig(foreground_color=fg))
    f.filter("x")
    result = f.filter("\x1b[1m")
...
def test_petscii_brightness_contrast_applied() -> None:
    f_full = PetsciiColorFilter(brightness=1.0, contrast=1.0)
    f_dim = PetsciiColorFilter(brightness=0.5, contrast=0.5)
