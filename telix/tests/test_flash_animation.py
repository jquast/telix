...
class TestLerpHsv:
    ...
    def test_endpoints(self):
        a = (0.0, 0.5, 0.8)
        b = (120.0, 1.0, 1.0)
        assert lerp_hsv(a, b, 0.0) == a
        assert lerp_hsv(a, b, 1.0) == b

    def test_midpoint(self):
        h, s, v = lerp_hsv((0.0, 0.0, 0.0), (120.0, 1.0, 1.0), 0.5)
        assert abs(h - 60.0) < 0.01
        assert abs(s - 0.5) < 0.01
        assert abs(v - 0.5) < 0.01

    def test_shortest_arc_wraps_through_zero(self):
        h, _, _ = lerp_hsv((350.0, 1.0, 1.0), (10.0, 1.0, 1.0), 0.5)
        assert abs(h) < 1.0 or abs(h - 360.0) < 1.0

    def test_shortest_arc_does_not_go_long_way(self):
        h, _, _ = lerp_hsv((350.0, 1.0, 1.0), (10.0, 1.0, 1.0), 0.25)
...
class TestFlashColor:
    ...
    def test_returns_valid_hex(self):
        for elapsed in [0.0, 0.05, 0.15, 0.25, 0.4, 0.6]:
            result = flash_color("#3388cc", elapsed)
...
class TestVitalBarFlash:
    ...
    def test_flash_elapsed_past_duration_same_as_normal(self):
        frags_normal = vital_bar(50, 100, 20, "hp", flash_elapsed=-1.0)
        frags_past = vital_bar(50, 100, 20, "hp", flash_elapsed=1.0)
